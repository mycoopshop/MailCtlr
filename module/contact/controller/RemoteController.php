...
class RemoteController
{
    ...
    public function deleteDuplicateAction()
    {
        ...
        foreach ($d as $c) {
            ...
            Contact::delete($c['id']);
            $is = Iscrizioni::query(['contatto_id' => $c['id']]);
            $cs = Coda::query(['contact_id' => $c['id']]);
            foreach ($is as $i) {
                Iscrizioni::delete($i->id);
            }
            foreach ($cs as $i) {
                Coda::delete($i->id);
            }
    ...
    */
    ...
    {
        $cs = Contact::query(['token_c' => 0, 'limit' => 10]);
        $reply = '';
        foreach ($cs as $c) {
    ...
    */
    ...
    public function processAction()
    {
        ...
        if (!$server) {
            $reply = 'non ci sono server'."\r\n";
        }
        if (!isset($server->host)) {
            $reply = 'il server non ha un host valido'."\r\n";
        }
