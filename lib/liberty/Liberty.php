...
class Liberty
{
    ...
    public static function request_parse($request_uri, $php_self, $default_controller = 'Home', $default_action = 'index')
    {
        ...
        $e = !in_array($d[0], ['', 'index.php', false, null]) ? ucfirst($d[0]) : $default_controller;
        $f = isset($d[1]) && !in_array($d[1], ['', 'index.php', false, null]) ? strtolower($d[1]) : $default_action;
        $g = [];
        if (count($d) > 2) {
            foreach (array_slice($d, 2) as $i => $v) {
                if ($i % 2 == 0) {
                    $g[$v] = null;
                    $h = $v;
                } else {
